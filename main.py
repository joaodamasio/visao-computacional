...
#Para fazer a captura ao vivo primeiro é preciso coletar imagens de vídeo
#O index se refere a câmera que irá capturar as imagens
...
#Posição dos pontos do olho esquerdo e direito
p_olho_esq = [385, 380, 387, 373, 362, 263]
p_olho_dir = [160, 144, 158, 153, 33, 133]
p_olhos = p_olho_esq + p_olho_dir

#Posição dos pontos dos lábios
p_boca = [82, 87, 13, 14, 312, 317, 78, 388]
...
#Função para a calcular a distancia euclidiana dos olhos
def calculo_ear(face, p_olho_dir, p_olho_esq):
    try:
        ...
        ear_dir = (np.linalg.norm(face_dir[0]-face_dir[1])+np.linalg.norm(face_esq[2]-face_dir[3]))/(2*(np.linalg.norm(face_dir[4]-face_dir[5])))
...
#Função para calcular a distancia euclidiana dos labios
...
#Fazendo mapeamento dos pontos do rosto
with mp_face_mesh.FaceMesh(min_detection_confidence= 0.5, min_tracking_confidence=0.5) as facemesh:
    while cap.isOpened():
        sucesso, frame = cap.read()
        
        #Verifica se a captura esta acontecendo o nao
        if not sucesso:
            print("Ignorando o frame vazio da camera")
            continue
        
        #Coletando, manipulando, e processando o frame
        ...
        try:
            #Mostrando os pontos mapeados através de um laço de repetição e ajustando cores e medidas dos pontos
            for face_landmarks in saida_facemesh.multi_face_landmarks:
                mp_drawing.draw_landmarks(frame, face_landmarks, mp_face_mesh.FACEMESH_CONTOURS,
                                        landmark_drawing_spec = mp_drawing.DrawingSpec(color= (102,255,102), thickness=1, circle_radius= 1),
                ...
                #Calculando o número de piscadas
